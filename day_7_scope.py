...
def fun(
    nums=None,
):  # None is an immutable value. None point is used for same memory address.  Dont give mutable values as default values
    if nums is None:
        nums = []
    nums.append(10)
    print(nums)
...
x2 = [2, 3]
x3 = x1
print(x1 == x2)  # checks the value
print(
    x1 is x2
)  # returns false coz they are not pointing at the same memory address/ checks memory address
